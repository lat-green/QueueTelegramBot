...
@Transactional
class InQueueState(val positionRepository: PositionRepository,
    ...
    fun findFirstFreeNumber(positions: List<Position>) : Int {
        if (positions.isEmpty())
            return 0

        var i = 0
        while (positions[i].number == i && i < positions.size)
            i++
        if (positions.last().number > i)
            return i - 1
        return i
    }
}
